...
final class Resolver(site: Site, context: Context) {
  ...
  def resolve(document: Markup.Document): IO[Markup.Document] = for {
    description <- resolveInlines(document.metadata.description)
    author <- document.metadata.author map (a => resolve(a) map (Some(_))) getOrElse resolve(Author(site.owner)).map(Some(_))
    content <- resolveBlocks(document.content)
    sections <- resolveSections(document.sections)
  } yield document.copy(
    metadata = document.metadata.copy(description = description, author = author),
    content = content,
    sections = sections)
